// Node tooling shared by next.config.ts and the server-only auth configuration.
...
function getTailscaleHosts(): string[] {
  ...
  for (const bin of TAILSCALE_BINS) {
    try {
      // Optional host-installed CLI; never trace this binary into the app bundle.
      ...
      // Trust only this machine's own names, not other tailnet members.
      ...
      // Tailscale CLI is optional; try the next executable.
    }
  }
...
export function getLocalNetworkHosts(): string[] {
  ...
  if (process.env.NEXT_PUBLIC_URL) {
    hosts.add(new URL(process.env.NEXT_PUBLIC_URL).hostname)
  }
 ...
 * Includes this machine's discovered LAN and Tailscale hosts.
 */
export function getAllowedDevOrigins(): string[] {
  return ['localhost', '127.0.0.1', ...getLocalNetworkHosts()]
}
